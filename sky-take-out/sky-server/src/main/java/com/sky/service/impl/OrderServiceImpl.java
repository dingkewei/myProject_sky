...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderVO getDetail(Long id) {
        ...
        log.info("查询到的订单信息：{}", orders);
        // 查询该订单对应的菜品/套餐明细
        List<OrderDetail> orderDetails = orderDetailMapper.getByOrderId(id);
        log.info("查询到的明细{}", orderDetails);
        // 将该订单及其详情封装到OrderVO并返回
        OrderVO orderVO = new OrderVO();
        BeanUtils.copyProperties(orders, orderVO);
        orderVO.setOrderDetailList(orderDetails);
        log.info("返回的VO{}", orderVO);
    ...
    public void cancelById(Long id) throws Exception {
        ...
        Integer status = ordersById.getStatus();
        if (status>2){
        ...
        if (status.equals(Orders.TO_BE_CONFIRMED)){
            ...
            weChatPayUtil.refund(
                    ordersById.getNumber(), //商户订单号
                    ordersById.getNumber(), //商户退款单号
    ...
    public void repetition(Long id) {
        ...
        List<OrderDetail> orderDetails = orderDetailMapper.getByOrderId(id);
        // 将订单详情对象转换为购物车对象
        List<ShoppingCart> shoppingCartsList = orderDetails.stream().map(x -> {
            ShoppingCart shoppingCart = new ShoppingCart();
    ...
    public void confirm(OrdersConfirmDTO ordersConfirmDTO) {
        ...
        Orders orders = Orders.builder()
                ...
                .estimatedDeliveryTime(resolveEstimatedDeliveryTime(ordersDB))
                .build();
    ...
    private LocalDateTime resolveEstimatedDeliveryTime(Orders orders) {
        if (orders != null && orders.getEstimatedDeliveryTime() != null) {

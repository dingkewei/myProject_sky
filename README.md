...
一个基于 Spring Boot + Vue2 + TypeScript 的外卖系统毕业设计项目。在传统的菜品、套餐、订单、员工、数据统计等业务模块基础上，进一步引入本地大模型能力，让 AI 不再只是“聊天入口”，而是能够理解业务语义、调用真实业务能力、返回可追踪执行结果的智能助手。
...
- [AI 管理台页面](./project-sky-admin-vue-ts/src/views/aiService/index.vue)

### 2. 用户端 AI 智能客服与智能下单

- 支持查询当前订单状态和历史订单记录。
- 支持从自然语言中抽取商品名、数量、备注等信息，自动组装下单请求。
- 默认地址、购物车恢复、失败提示等业务细节也被纳入 AI 工具调用流程，而不是只停留在“回答建议”层。

重点实现：

- [UserAiCustomerServiceImpl.java](./sky-take-out/sky-server/src/main/java/com/sky/service/impl/UserAiCustomerServiceImpl.java)
- [用户侧 AI 接口](./sky-take-out/sky-server/src/main/java/com/sky/controller/user/AiCustomerServiceController.java)
...
- AI：Ollama，本地模型默认 `deepseek-r1:7b`
- 数据层：MySQL
...
如果你要在 GitHub、答辩或视频演示里突出 AI 亮点，可以直接展示下面这些场景：
...
- 真实开发配置文件未上传，仓库使用示例配置文件代替。
- `sky-take-out` 原有独立 Git 历史已做本地备份后并入总仓库，方便统一展示前后端。
- `node_modules`、`dist`、日志、IDE 配置和论文辅助目录默认不会提交。

更多 GitHub 展示文案和上传建议见：
